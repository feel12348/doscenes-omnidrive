...
from ..utils.misc import load_model, _ensure_token_embeddings, _assert_token_embeddings
...
def fill_drivecode_numbers(text, numbers, placeholder):
    if len(numbers) == 0:
        return text
    filled = text
    for idx, value in enumerate(numbers):
        number_text = f"{value:+.2f}"
        if placeholder and placeholder in filled:
            filled = filled.replace(placeholder, number_text, 1)
        else:
            filled += number_text
...
class Petr3D(MVXTwoStageDetector):
    ...
    def __init__(self,
                 ...
                 drivecode_number_token=DEFAULT_NUMBER_TOKEN,
        ...
        self.drivecode_number_token = drivecode_number_token
        self.drivecode_number_token_id = drivecode_number_token_id
        ...
        if tokenizer is not None:
            ...
            if self.drivecode_number_token_id is not None:
                self.drivecode_number_token = self.tokenizer.convert_ids_to_tokens(
                    self.drivecode_number_token_id)
            if self.enable_drivecode_numbers:
                if self.drivecode_number_token_id is None:
                    if self.tokenizer.convert_tokens_to_ids(
                            self.drivecode_number_token) == self.tokenizer.unk_token_id:
                        self.tokenizer.add_special_tokens(
                            {'additional_special_tokens': [self.drivecode_number_token]})
                    self.number_token_id = self.tokenizer.convert_tokens_to_ids(
                        self.drivecode_number_token)
                else:
                    self.number_token_id = self.drivecode_number_token_id
        ...
        if lm_head is not None:
            ...
            self.lm_head = _ensure_token_embeddings(self.lm_head, vocab_size)
            _assert_token_embeddings(self.lm_head, vocab_size)
            lm_config_owner = self.lm_head.get_base_model() if hasattr(self.lm_head, 'get_base_model') else self.lm_head
            for module in self.lm_head.modules():
                module.drivecode_enabled = self.enable_drivecode_numbers
                module.number_token_id = self.number_token_id
                if hasattr(module, 'config'):
                    module.config.drivecode_enabled = self.enable_drivecode_numbers
                    module.config.number_token_id = self.number_token_id
            if os.environ.get('DRIVECODE_DEBUG', '0') == '1':
                embed_size = lm_config_owner.get_input_embeddings().num_embeddings
                model_embed_size = (
                    lm_config_owner.model.embed_tokens.num_embeddings
                    if hasattr(lm_config_owner, 'model') and hasattr(lm_config_owner.model, 'embed_tokens')
                    else None)
                lm_size = lm_config_owner.lm_head.out_features if hasattr(lm_config_owner, 'lm_head') else None
                print(
                    f"[DriveCodeDebug][model-init] tokenizer_len={len(self.tokenizer)} "
                    f"number_token_id={self.number_token_id} "
                    f"config_number_token_id={getattr(lm_config_owner.config, 'number_token_id', None)} "
                    f"drivecode_enabled={getattr(lm_config_owner.config, 'drivecode_enabled', None)} "
                    f"embed_size={embed_size} model_embed_size={model_embed_size} lm_head={lm_size}",
                    flush=True)
    ...
    def forward_pts_train(self,
        ...
        if self.with_lm_head:
            ...
            drivecode_active = bool(self.enable_drivecode_numbers)
            vlm_loss = self.lm_head(
                ...
                drivecode_enabled=drivecode_active,
                number_token_id=self.number_token_id if drivecode_active else None,
                use_cache=False)
            losses.update(vlm_loss=vlm_loss[0])
            ce_loss = getattr(vlm_loss, 'ce_loss', None)
            number_loss = getattr(vlm_loss, 'number_loss', None) if drivecode_active else None
            number_count = getattr(vlm_loss, 'number_count', None) if drivecode_active else None
            if ce_loss is None and isinstance(vlm_loss, tuple) and len(vlm_loss) >= 4:
                ce_loss = vlm_loss[-3]
                if drivecode_active:
                    number_loss = vlm_loss[-2]
                    number_count = vlm_loss[-1]
            loss_owner = self.lm_head.get_base_model() if hasattr(self.lm_head, 'get_base_model') else self.lm_head
            if ce_loss is None:
                ce_loss = getattr(loss_owner, 'last_ce_loss', None)
            if drivecode_active and number_loss is None:
                number_loss = getattr(loss_owner, 'last_number_loss', None)
            if drivecode_active and number_count is None:
                number_count = getattr(loss_owner, 'last_number_count', None)
            if ce_loss is None:
                ce_loss = vlm_loss[0].detach()
            if ce_loss is not None:
                ce_loss = torch.nan_to_num(
                    ce_loss, nan=0.0, posinf=1e4, neginf=-1e4)
            if drivecode_active and number_loss is None:
                number_loss = vlm_loss[0].detach().new_tensor(0.0)
            if drivecode_active and number_count is None:
                if self.enable_drivecode_numbers and self.number_token_id is not None and vlm_labels is not None:
                    label_hits = (vlm_labels == self.number_token_id).sum()
                    if number_values is not None:
                        value_hits = (~torch.isnan(number_values)).sum()
                        label_hits = torch.minimum(label_hits, value_hits)
                    number_count = label_hits.detach().to(vlm_loss[0].device).float()
                else:
                    number_count = vlm_loss[0].detach().new_tensor(0.0)
            if ce_loss is not None:
                losses.update(vlm_ce=ce_loss)
            if drivecode_active and number_loss is not None:
                losses.update(vlm_number=number_loss)
            if drivecode_active and number_count is not None:
                losses.update(vlm_number_count=number_count)
    ...
    def forward_train(self,
        ...
        if self.tokenizer is not None:
            ...
            if self.enable_drivecode_numbers and number_values is not None:
    ...
    def simple_test_pts(self, img_metas, **data):
        ...
        if should_generate:
            ...
            for i, input_ids in enumerate(data['input_ids'][0]):
                ...
                number_generator = self.lm_head
                if not hasattr(number_generator, 'generate_with_numbers') and hasattr(self.lm_head, 'get_base_model'):
                    number_generator = self.lm_head.get_base_model()
                if self.enable_drivecode_numbers and hasattr(number_generator, 'generate_with_numbers'):
                    output_ids, output_numbers = number_generator.generate_with_numbers(
                        inputs=input_ids,
                        images=vision_embeded,
                        max_new_tokens=320,
                    ...
                    decoded = [
                        fill_drivecode_numbers(
                            text,
                            output_numbers[batch_idx],
                            self.drivecode_number_token)
                        for batch_idx, text in enumerate(decoded)
                    ]
                else:
