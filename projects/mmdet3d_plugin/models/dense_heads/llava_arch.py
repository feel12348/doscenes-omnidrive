    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        for batch_idx, cur_input_ids in enumerate(input_ids):
            ...
            if num_images == 0:
                ...
                number_token_id = (
                    getattr(self, 'number_token_id',
                            getattr(self.config, 'number_token_id', None))
                    if drivecode_active else None)
                new_number_masks.append(
                    cur_input_ids == number_token_id
                    if number_token_id is not None
                    else torch.zeros_like(cur_input_ids, dtype=torch.bool))
                cur_image_idx += 1
                continue
            ...
            number_token_id = (
                getattr(self, 'number_token_id',
                        getattr(self.config, 'number_token_id', None))
                if drivecode_active else None)
            cur_number_mask_noim_cat = torch.zeros(
                cur_input_ids_noim_cat.shape,
                dtype=torch.bool,
                device=cur_input_ids_noim_cat.device)
            if drivecode_active and number_values is not None and number_token_id is not None and hasattr(self, 'number_projector'):
                number_positions = torch.where(cur_input_ids_noim_cat == number_token_id)[0]
                cur_number_mask_noim_cat[number_positions] = True
            ...
            cur_number_masks_no_im = torch.split(cur_number_mask_noim_cat, split_sizes, dim=0)
            
            cur_new_input_embeds = []
            cur_new_labels = []
            cur_new_number_masks = []

            for i in range(num_images + 1):
                cur_new_input_embeds.append(cur_input_embeds_no_im[i])
                cur_new_labels.append(cur_labels_noim[i])
                cur_new_number_masks.append(cur_number_masks_no_im[i])
                if i < num_images:
                    cur_image_features = image_features[cur_image_idx]
                    cur_image_idx += 1
                    cur_new_input_embeds.append(cur_image_features)
                    cur_new_labels.append(torch.full((cur_image_features.shape[0],), IGNORE_INDEX, device=cur_labels.device, dtype=cur_labels.dtype))
                    cur_new_number_masks.append(torch.zeros(
            ...
            cur_new_number_masks = torch.cat(cur_new_number_masks)

            new_input_embeds.append(cur_new_input_embeds)
            new_labels.append(cur_new_labels)
            new_number_masks.append(cur_new_number_masks)
        ...
        new_number_masks_padded = torch.zeros((batch_size, max_len), dtype=torch.bool, device=new_labels[0].device)
        attention_mask = torch.zeros((batch_size, max_len), dtype=attention_mask.dtype, device=attention_mask.device)
        position_ids = torch.zeros((batch_size, max_len), dtype=position_ids.dtype, device=position_ids.device)

        for i, (cur_new_embed, cur_new_labels, cur_new_number_masks) in enumerate(zip(new_input_embeds, new_labels, new_number_masks)):
            ...
            if cur_len > 0:
                ...
                new_number_masks_padded[i, :cur_len] = cur_new_number_masks
                attention_mask[i, :cur_len] = True
                position_ids[i, :cur_len] = torch.arange(0, cur_len, dtype=position_ids.dtype, device=position_ids.device)

        new_input_embeds = torch.stack(new_input_embeds_padded, dim=0)
        self.last_number_token_mask = (
            new_number_masks_padded if drivecode_active else None)
