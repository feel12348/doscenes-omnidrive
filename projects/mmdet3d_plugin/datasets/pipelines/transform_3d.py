...
class LoadAnnoatationVQA():
    def __init__(
        ...
        if drivecode_number_token_id is not None:
            drivecode_number_token = self.tokenizer.convert_ids_to_tokens(
                drivecode_number_token_id)
        ...
        if self.enable_drivecode_numbers:
            if self.drivecode_number_token_id is None:
                ...
                number_token_id = self.tokenizer.convert_tokens_to_ids(
                    self.drivecode_number_token)
            else:
                number_token_id = self.drivecode_number_token_id
        self.n_gen = n_gen
    ...
    def __call__(self, results):
        ...
        if 'gt_planning' in results.keys():
            ...
            has_planning_traj = (
                len(planning_traj) == 6
                if self.planning_steps is None else len(planning_traj) == self.planning_steps
            )
            if has_planning_traj:
                if self.enable_drivecode_numbers:
                    formatted_points = ', '.join(
                        f"({self.drivecode_number_token}, {self.drivecode_number_token})"
        ...
        if 'gt_planning' in results.keys() and has_planning_traj:
        ...
        first_prompt = planning_prompt if ('gt_planning' in results.keys() and has_planning_traj) else prompt
        ...
        if os.environ.get('DRIVECODE_DEBUG', '0') == '1' and not getattr(self, '_drivecode_debug_printed', False):
            number_token_id = self.tokenizer.convert_tokens_to_ids(self.drivecode_number_token)
            input_count = int((input_ids == number_token_id).sum().item())
            label_count = int((vlm_labels == number_token_id).sum().item())
            value_count = 0 if number_values is None else int(number_values.reshape(-1).shape[0])
            print(
                f"[DriveCodeDebug][data] token={self.drivecode_number_token!r} "
                f"id={number_token_id} input_count={input_count} "
                f"label_count={label_count} value_count={value_count} "
                f"input_len={input_ids.numel()}",
                flush=True)
            self._drivecode_debug_printed = True
